"""阿里云监控全局资源处理模块，负责获取云监控告警规则、告警联系人、联系组等全局资源信息。"""
import logging
from typing import Dict, List, Any

logger = logging.getLogger(__name__)

class MonitoringGlobalAssetCollector:
    """阿里云监控全局资源收集器"""
    ...
    def get_alarm_rules(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"获取到第一页告警规则数据，共 {len(rules)} 条（全局资源）")
    ...
    def get_all_monitoring_global_assets(self) -> Dict[str, Any]:
        """
        获取所有监控全局资源（告警规则、告警联系人、联系组、站点监控等）
        
        全局资源定义：在不同区域调用API返回完全一致的数据

        Returns:
            Dict[str, Any]: 所有监控全局资源
        """
        logger.info("获取所有阿里云监控全局资源")
        ...
        logger.info(f"已获取 {len(alarm_rules)} 条告警规则, {len(alarm_contacts)} 个告警联系人, {len(alarm_contact_groups)} 个告警联系组, {len(site_monitors)} 个站点监控（全局资源）")
