"""
AWS IAM全局资源处理模块，负责获取用户、角色、策略等IAM全局资源信息。
IAM是AWS的全局服务，在不同区域获取到的数据是一致的。
"""
import boto3
import logging
...
class IAMGlobalAssetCollector:
    ...
    def get_all_iam_global_assets(self) -> Dict[str, Dict[str, Dict[str, Any]]]:
        """
        获取所有IAM全局资源，按类型分开并返回单个资源项

        Returns:
            Dict[str, Dict[str, Dict[str, Any]]]: 按类型分开的IAM全局资源，每个资源项单独存储
        """
        logger.info("开始收集所有IAM全局资源")
